...
const createTileMap = (scene: MountainScene, totalHeight: number): void => {
    ...
    createMountains(15, groundLayer, map, tileset);
...
const createMountains = (startX: number, 
    ...
    let x = startX;
    let prevType = 'flat';
    const maxWallHeight = 10;
    const maxFlatLength = 10;
    ...
    while(x < map.width){// make one mountain
        ...
        let params;
        params = buildMountainUp(mountainHeight, maxWallHeight, maxFlatLength, prevType, x, y, layer, map, tileset);
        params = buildMountainDown(mountainHeight, mountainEndHeight, maxWallHeight, params.prevType, maxFlatLength, params.x, params.y, layer, map, tileset);
        x = params.x;
        y = params.y;
...
const fillBelow = (x: number, y: number, layer: Phaser.Tilemaps.DynamicTilemapLayer, map: Phaser.Tilemaps.Tilemap): void => {
        ...
        for(let i=y-1; i>=0; i--){
            ...
            const myTile = new Phaser.Tilemaps.Tile(layer.layer,
                idx, 
                0, 0, 64, 64, 64, 64);
            map.putTileAt(myTile, x, mountainHeightMap(i), true, layer);                
        }

}

const buildMountainUp = (mountainHeight: number, maxWallHeight: number, maxFlatLength: number, prevType: string, x: number, y: number, 
                   layer: Phaser.Tilemaps.DynamicTilemapLayer, 
                   map: Phaser.Tilemaps.Tilemap,
    ...
    while(!reachedTop){
        ...
        if(prevType==='flat'){// make a vertical wall
            ...
            map.putTileAt(botTile, x, mountainHeightMap(y-1), true, layer);
            ...
            for(let i=0; i < wallHeight - 1; i++){
                ...
                map.putTileAt(wallTile, x, mountainHeightMap(y+i), true, layer);
            ...
            map.putTileAt(topTile, x, mountainHeightMap(y + wallHeight-1), true, layer);
            ...
            for(let i=0; i < flatLength; i++){
                map.putTileAt(flatTile, x + i, mountainHeightMap(y), true, layer);
...
const buildMountainDown = (mountainHeight: number, mountainEndHeight: number, maxWallHeight: number, prevType: string, maxFlatLength: number, x: number, y: number, 
    layer: Phaser.Tilemaps.DynamicTilemapLayer, 
    map: Phaser.Tilemaps.Tilemap,
    tileset: Phaser.Tilemaps.Tileset) => {
    ...
    while(!reachedBottom){
        ...
        if(prevType==='flat'){// make a vertical wall
            ...
            topTile.properties = tileset.getTileProperties(idx);
            map.putTileAt(topTile, x, mountainHeightMap(y), true, layer);
            ...
            for(let i=0; i < wallHeight - 1; i++){
                ...
                wallTile.properties = tileset.getTileProperties(idx);
                map.putTileAt(wallTile, x, mountainHeightMap(y-1-i), true, layer);
            ...
            botTile.properties = tileset.getTileProperties(idx);
            map.putTileAt(botTile, x, mountainHeightMap(y-wallHeight), true, layer);
...
const mountainHeightMap = (y: number) => {
    return 99 - y
}
